...
最近一次全量结果：typecheck 通过；单测 server 156 / story-engine 12 / web 26；e2e 31 条通过。
...
这一批前四条来自 2026-09-08 站在各类用户角度做的一次审计，都是"界面在说谎"或"用户为不是自己的错付费"这一类，优先于新功能：

1. **中断不该收费**：读者点 `停止生成` 后服务端毫不知情——它把生成跑完、记一次 `success`、把整段落库。界面说已停止，配额少一次，下次打开还会看到那一段。流式路由要监听 `request.raw` 的关闭并把中断传给 `provider.streamNarrative`；中断分支也要刷新配额显示。
2. **流式失败的回落会调两次模型**：`reader-client.tsx` 回落到非流式端点时又消耗一次 `generationLimiter` 并再调一次模型。配额不双扣，但突发额度和 token 都花两份。
3. **把真话透传到界面**：409/400 的 `error` 与 `issues` 被 `lib/api.ts` 丢掉，重复 ID 只显示「创建故事失败」；故事 ID 的规则（小写、≥3 位、首尾非连字符）也该写在输入框旁边。
4. **配额重置说清楚**：`created_date` 按 UTC 日切，东八区的「明天再来」实际是早上 8 点。`TurnQuota` 需要一个重置时间字段。
5. **探索页的秩序**：现在按故事 ID 字母序，没有搜索、筛选、分页，也没按热度/最近排序——读者数已经算出来了却没当排序键用。
6. **阅读预期**：卡片仍然不说这故事大概能读多久。锚点数量 + `defaultSegmentLength` 够给一个诚实的区间，但注意锚点现在只在作者视图里，公开视图要另给一个"锚点条数"之类的计数，不能把大纲带出去。
7. **`SessionSegment` 缓存与作者预设优先**：现在每次推进都实打实调模型。按 `ARCHITECTURE.md` §3.2 的三级优先级（预设 → 已生成 → 现生成）做，才对得起 `scripted` 这个模式名。
8. **按故事的成本视图**：`generation_usage` 每行都有 `userId` 和 `storyId`，管理台却只给全站当日总量，看不出是哪个故事在烧钱。
9. **`路线分歧` 与 `NPC 发问` 两种节点**只能由模型识别（状态里读不出来），这是能力边界，不是遗漏；要做只能加一次轻量的判定调用。

## 已知问题

- `DAILY_TURN_QUOTA` 填成非数字会变成 `NaN`，配额检查从此永不拒绝（`main.ts` 里其他限流参数有 `Number.isFinite` 保护，这个没有）。
- 非 production 且未设 `ADMIN_TOKEN` 时 `/api/admin` 对所有人开放（`main.ts` 只在 production 下强制），而默认 `HOST=0.0.0.0`。
- 作者试玩消耗作者自己的每日配额，试玩会话也和真实阅读混在同一个 `继续` 列表里、没有标记。insights 排除作者、配额不排除——同一件事两处口径相反。
