...
最近一次全量结果：typecheck 通过；单测 server 158 / story-engine 12 / web 26；e2e 31 条通过。
...
这一批前三条来自 2026-09-08 站在各类用户角度做的一次审计，都是"界面在说谎"或"用户为不是自己的错付费"这一类，优先于新功能：

1. **流式失败的回落会调两次模型**：`reader-client.tsx` 回落到非流式端点时又消耗一次 `generationLimiter` 并再调一次模型。配额不双扣，但突发额度和 token 都花两份。
2. **把真话透传到界面**：409/400 的 `error` 与 `issues` 被 `lib/api.ts` 丢掉，重复 ID 只显示「创建故事失败」；故事 ID 的规则（小写、≥3 位、首尾非连字符）也该写在输入框旁边。
3. **配额重置说清楚**：`created_date` 按 UTC 日切，东八区的「明天再来」实际是早上 8 点。`TurnQuota` 需要一个重置时间字段。
4. **探索页的秩序**：现在按故事 ID 字母序，没有搜索、筛选、分页，也没按热度/最近排序——读者数已经算出来了却没当排序键用。
5. **阅读预期**：卡片仍然不说这故事大概能读多久。锚点数量 + `defaultSegmentLength` 够给一个诚实的区间，但注意锚点现在只在作者视图里，公开视图要另给一个"锚点条数"之类的计数，不能把大纲带出去。
6. **`SessionSegment` 缓存与作者预设优先**：现在每次推进都实打实调模型。按 `ARCHITECTURE.md` §3.2 的三级优先级（预设 → 已生成 → 现生成）做，才对得起 `scripted` 这个模式名。
7. **按故事的成本视图**：`generation_usage` 每行都有 `userId` 和 `storyId`，管理台却只给全站当日总量，看不出是哪个故事在烧钱。
8. **`路线分歧` 与 `NPC 发问` 两种节点**只能由模型识别（状态里读不出来），这是能力边界，不是遗漏；要做只能加一次轻量的判定调用。
