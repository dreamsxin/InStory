...
最近一次全量结果：typecheck 通过；单测 server 197 / story-engine 14 / web 42；e2e 36 条通过。
...
- **卡片说得出这故事有多长**：`GET /api/stories` 现在每条带 `plannedBeats`（作者标了「必经」或「可作为结局」的锚点条数）和 `segmentTargetWords`（那档篇幅真正写进提示词的字数，`SEGMENT_LENGTH_GUIDES` 是唯一来源），卡片上写成「主线 3 个节点 · 每段约 1200 字 · 约 9 分钟起」。是「起」而不是估算总量：一个节点读者可能来回几段，所以这是下限。没有预设节点的故事不编数字，直说长度由读者和 AI 一起决定。给的是计数和字数，锚点标题与说明仍然只在作者视图里——那是大纲，等于结局。作者写了预设正文之后，卡片不再只给估算：`presetPassages` / `presetWords` 是数出来的（`countPresetPassages`，`length(json_extract(...))` 按字符数），卡片改说「作者写好 3 段共 1180 字（约 3 分钟）· 之后由 AI 接着写」，把"已经写好的"和"按篇幅档估的"分开——前者是事实，后者只是下限。只对剧本模式的故事报这两个数：另两档存了也不会发给读者，替它们数一遍等于告诉读者作者给他写了两章而实际一个字都到不了。同样只给计数与字数，正文本身不上书架（测试断言书架响应里搜不到那段文字）。
...
- **作者看得见读者在他写的第几段停下**：预设正文上一条只做完了"写"和"发"，作者仍然不知道读者有没有读完他写的那几段——这正是锚点当年的毛病（只进提示词，什么都不回来）。`StoryReadingInsight` 现在多一个 `passageReach`（`summarizeStories` 里第三条聚合，和 `anchorReach`、`readers` 同一套逐故事排除作者本人、按读者去重），预设正文编辑器的每一行因此写出「2 位读者读到这一段」，顺着往下读就是一条流失曲线：读者在哪一段停下，作者就该回头看那一段。和锚点的区别写在注释里：锚点数字是"模型声称推进了它"，而这个是准数——是服务端决定把哪一段发出去的，没有上报环节。沉默规则沿用锚点那套：没有别人读过之前整张表不出数字（"没有人读到"对每个没人打开的故事都成立，印在每一行上像是在评判文笔），新加的行没有 id 也就没有数字。但 0 在这里是要说的：一段没人读到，意思是读者在它之前就停了，那正是作者从正文里看不出来的东西。汇总句（故事行上那句「1 位读者读了 2 段：…」）没有跟着加预设正文的部分——作者要动手的地方是那一行本身，不是标题旁边再堆一句。
- **读者可以让阅读器发声，但要他自己开口要**：纯文本读到关键节点时，界面只能靠一枚标签说"这里有事发生"，眼睛已经在正文上的人往往看不见。现在阅读器有四个音效提示（`lib/reading-sounds.ts`）：读完一段是一声很轻的低音，停在关键节点时换一种——有人向你发问是两声上行、危机逼近是两声下行、其余的关键节点（线索、转折、关系变化、路线分歧）共用一声双音铃。一段只响一次：有关键节点时就用那一种，不再叠加翻页声。四种而不是六种是有意的：六种可分辨的声音等于要读者学一套语言，而那四类的共同意思本来就是"这里发生了什么"。三个决定值得写下来：**用 Web Audio 现场合成，不带音频文件**——仓库不多一个二进制资产，读者也不必先下载才能读，而且不存在"一段录得不好的音"这种失败；**只跟着读者自己的操作响**，没有自动播放、没有循环、没有背景音乐（浏览器也正好要求先有手势才允许出声）；**默认关**，在安静的房间里打开一个故事就该是安静的，所以偏好初值是 `off`，读者从工具坞的 `开音效` 自己打开——按下的那一次就是浏览器要的那个手势，也顺便让他听到自己刚同意了什么。开关放在工具坞而不是只放在 `版式` 面板里：工具坞是 `沉浸阅读` 下唯一还在的那一条，静音永远只差一次点击；`版式` 面板里另有一组「音效 关 / 开」并写清这些声音是什么。偏好跟着字号行距一起存在本机（`ReadingPrefs.sound`），老数据里没有这个字段就落在"关"——那和"从没被问过"是同一个答案。取不到 AudioContext（浏览器不支持、被策略挡下、设备没有声音）时静默跳过：一次没响的提示不值得在正在读书的人面前弹错误。声音配方是纯数据，所以没有喇叭也能测：单测钉住每个音都在 120–2000Hz、不超过 0.4 秒、峰值不超过 0.1，翻页声是四者里最轻的（它响得最勤）。
...
- `apps/web/src/components/reader-client.tsx`：阅读器全部交互。`lib/reading-prefs.ts` 是读者自己的阅读偏好（字号、行距、行宽、音效开关，存本机），`lib/reading-sounds.ts` 是那几个音效的配方与播放。
...
按我判断的优先级排。第 1 条是能力边界，第 2 条是未定的产品决定，第 3、4 条是两笔性能与素材的账；原先排第一的「同一段不重复生成」被我降到第 5 条，理由写在那一条里——预设正文落地之后它剩下的命中场景都带着一个我不该替产品做的决定。

1. **`路线分歧` 只能由模型识别**，这是能力边界，不是遗漏：`choices` 在生成协议里是每段必给的 2 到 4 项，所以"有选项"根本不含"这里真的分叉了"的信息——照它派发 `fork` 等于给每个平淡段落都盖一个路线分歧的章，标签会立刻失效。要真做只能加一次轻量的判定调用。`NPC 发问` 已经不在这个名单里了：一句既带问号、又带"你"的对白是模型输出里现成的证据，服务端兜底现在读它。
2. **公开入戏角色（产品决定，未定）**：撤掉那个开关之后，「把自己的入戏角色公开、供作者选进故事」这件事变成了一个明确的产品问题而不是一个待接线的字段。要做得先答三问：读者在哪里浏览别人的角色；作者选角时是引用还是复制（现在故事内演员是复制快照）；角色被别人用过之后原主还能不能改、改了算谁的。答不上来就不该有那个开关。
3. **书架的关键词与排序还没进索引**：查询已经在服务端了，但匹配到的那一批仍然整份读进内存再排序取窗口（`searchPublicStories` + `compareForShelf`）——因为排序键在 `reader_sessions` 里，而故事字段都藏在 `stories.payload` 的 JSON 里。几百条无所谓；真到几千条，就该把 `visibility` / `genre` / `title` 提成列并加索引，让 `ORDER BY` 和 `LIMIT` 交给 SQLite。要一起想的还有排序键怎么落库（物化一张 `story_reading_stats`？还是每次 join 聚合），这决定了「最近有人读」能不能进同一条 SQL。
4. **环境音与按装帧走的音景（未定，需要真素材）**：现在的音效是四个合成提示音，靠代码就能写对。再往前一步是"雨声、市集、地窖"这一类持续的环境音，那不是合成能糊过去的——一段听起来像白噪声的"雨"比没有雨更糟。要做得先解决三件事：素材从哪来且许可证清楚（仓库要开始装二进制资产，或者走 CDN）、谁决定哪个故事配哪种音景（作者选？按 `readingTheme` 映射？），以及移动端的电量与流量代价。做之前先想清楚它是不是真的比"读者自己放背景音乐"更好。
5. **同一段不重复生成（`SessionSegment` 缓存，降级）**：三级优先级里中间那一级仍然没有——同一处走第二遍还是照样调模型。但预设正文落地之后，它剩下的命中场景只有两个，而两个都不是纯技术问题：**跨读者**命中要求故事、状态、读者身份、输入全都相同，而"读者身份"进了提示词（正文里的"你"是一个具体的人），所以只有用默认演员进场的开场几段能撞上——那恰好正是作者现在可以自己写成预设正文的部分；**回溯重走**命中则要先回答"读者回溯之后再读同一段，他想看到一模一样的文字，还是一次新的？"——不少人回溯正是为了换一个结果，返回缓存会被当成坏了。再加上"命中要不要扣配额"（没花钱却收费不诚实，不收费则等于给"回溯循环 = 免费阅读"开了一条路）、作者改了世界或锚点之后旧正文如何失效、以及缓存表怎么不无限膨胀。这些答完之前不该动手：一个会偶尔重播旧段落、又说不清算不算次数的缓存，比每次都重新生成更难解释。
