...
- 产品规划更新为 UGC + 积分经济闭环：用户发布故事，读者消耗积分体验 AI 生成内容，创作者按有效体验获得积分。
- 文档补充故事创建字段：故事 ID、标题、标语、类型、世界前提、起点场景、世界规则、入戏体验模式、默认生成长度和 AI 自由度。
- 新增 `CreateStoryRequest` 共享契约和 `POST /api/admin/stories`，支持通过管理台创建最小故事草稿。
...
1. 实现故事发布状态与审核状态：草稿、待审核、已发布、已下架。
2. 扩展作者工具，支持编辑世界、作者角色、剧情锚点和预设后续。
3. 实现 `SessionSegment` 存储，生成内容按 `sessionId` 隔离，并预留 `userId`。
4. 拆分 `read_segment` 与 `intervention_turn`，让继续阅读返回完整小节。
5. 预留积分账户、积分流水和 AI 生成消耗记录。
6. 增加阅读设置：字号、行距、主题、滚动/分页偏好。
7. 增加 Web 阅读器交互测试和 `/admin` 页面级测试。
