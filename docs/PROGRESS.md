...
1. **配额重置说清楚**：`created_date` 按 UTC 日切，东八区的「明天再来」实际是早上 8 点。`TurnQuota` 需要一个重置时间字段。
2. **探索页的秩序**：现在按故事 ID 字母序，没有搜索、筛选、分页，也没按热度/最近排序——读者数已经算出来了却没当排序键用。
3. **阅读预期**：卡片仍然不说这故事大概能读多久。锚点数量 + `defaultSegmentLength` 够给一个诚实的区间，但注意锚点现在只在作者视图里，公开视图要另给一个"锚点条数"之类的计数，不能把大纲带出去。
4. **`SessionSegment` 缓存与作者预设优先**：现在每次推进都实打实调模型。按 `ARCHITECTURE.md` §3.2 的三级优先级（预设 → 已生成 → 现生成）做，才对得起 `scripted` 这个模式名。
5. **按故事的成本视图**：`generation_usage` 每行都有 `userId` 和 `storyId`，管理台却只给全站当日总量，看不出是哪个故事在烧钱。
