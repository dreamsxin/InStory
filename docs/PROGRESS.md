...
5. **环境音与按装帧走的音景（未定，需要真素材）**：现在的音效是四个合成提示音，靠代码就能写对。再往前一步是"雨声、市集、地窖"这一类持续的环境音，那不是合成能糊过去的——一段听起来像白噪声的"雨"比没有雨更糟。要做得先解决三件事：素材从哪来且许可证清楚（仓库要开始装二进制资产，或者走 CDN）、谁决定哪个故事配哪种音景（作者选？按 `readingTheme` 映射？），以及移动端的电量与流量代价。做之前先想清楚它是不是真的比"读者自己放背景音乐"更好。
