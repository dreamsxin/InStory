...
最近一次全量结果：typecheck 通过；单测 server 162 / story-engine 12 / web 35；e2e 31 条通过。
...
1. **探索页的秩序**：现在按故事 ID 字母序，没有搜索、筛选、分页，也没按热度/最近排序——读者数已经算出来了却没当排序键用。
2. **阅读预期**：卡片仍然不说这故事大概能读多久。锚点数量 + `defaultSegmentLength` 够给一个诚实的区间，但注意锚点现在只在作者视图里，公开视图要另给一个"锚点条数"之类的计数，不能把大纲带出去。
