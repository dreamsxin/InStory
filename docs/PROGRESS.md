...
1. 实现 `SessionSegment` 存储，生成内容按 `sessionId` 隔离，并预留 `userId`。
2. 拆分 `read_segment` 与 `intervention_turn`，让继续阅读返回完整小节。
3. 扩展作者工具，支持入戏体验模式和默认生成长度，并先完整支持 `共演入戏`。
4. 增加阅读设置：字号、行距、主题、滚动/分页偏好。
