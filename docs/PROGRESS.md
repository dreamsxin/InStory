...
- 移动端布局补充横屏短屏适配：手机横屏时首页底部导航切为左侧 rail，主内容保留独立滚动；阅读器横屏时正文、选项、输入区和状态侧栏按可用宽高重新分配。
- 将可通用 UI 控件迁移到 HeroUI v3：客户端和管理后台的卡片、按钮、输入、文本域、选择器、标签、头像、复选框等优先使用 HeroUI 组件，阅读正文排版继续保持自定义。
...
1. 改造阅读器：增加常驻“入戏”按钮和入戏面板。
2. 扩展服务端叙事模型：阅读段落、关键介入节点、入戏回写。
3. 增加阅读设置：字号、行距、主题、滚动/分页偏好。
4. 扩展作者工具，支持编辑世界、角色和剧情锚点。
5. 增加 Web 阅读器交互测试和 `/admin` 页面级测试。
