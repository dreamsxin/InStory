...
最近一次全量结果：typecheck 通过；单测 server 162 / story-engine 12 / web 35；e2e 32 条通过。
...
1. **阅读预期**：卡片仍然不说这故事大概能读多久。锚点数量 + `defaultSegmentLength` 够给一个诚实的区间，但注意锚点现在只在作者视图里，公开视图要另给一个"锚点条数"之类的计数，不能把大纲带出去。
2. **书架的服务端查询**：搜索、筛选、排序现在都在客户端做，前提是整份公开列表随首屏一起下发。故事多起来（一两百条以上）就得把这些下沉到 `GET /api/stories` 的 query 参数并分页，同时 `GET /api/stories/insights` 要跟着同一组条件走——现在两个接口各自算一遍公开故事集合。
3. **`SessionSegment` 缓存与作者预设优先**：现在每次推进都实打实调模型。按 `ARCHITECTURE.md` §3.2 的三级优先级（预设 → 已生成 → 现生成）做，才对得起 `scripted` 这个模式名。
4. **按故事的成本视图**：`generation_usage` 每行都有 `userId` 和 `storyId`，管理台却只给全站当日总量，看不出是哪个故事在烧钱。
