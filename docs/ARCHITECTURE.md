...
MVP 可先支持匿名用户和本地账号，支付系统只保留数据结构、积分流水和额度扣减逻辑。正式充值、提现和复杂分账不进入 MVP。
...
MVP 中作者工具应保持极简，优先支持基础表单编辑。普通用户必须能创建故事草稿，配置足以让 AI 生成第一轮体验的核心字段；复杂分支图、可视化编排和创作者收益看板放到 Phase 2。
...
type Story = {
  ...
  publishStatus: "draft" | "pending_review" | "published" | "archived";
  moderationStatus: "not_submitted" | "reviewing" | "approved" | "rejected";
};
```
...
`CreditLedgerEntry` 记录读者消耗或创作者获得积分，必须包含 `userId`、`storyId`、`sessionId`、`amount`、`reason`、`createdAt`。`GenerationUsage` 记录每次 AI 调用的模型、token、延迟和估算成本。MVP 可先只预留模型和接口，不接入真实支付。
...
MVP 阶段不追求可视化工作流，先保证作者能稳定创建并配置一部可试玩作品。第一版创建表单包含故事 ID、标题、标语、类型、世界前提、起点地点、起点场景、世界规则、入戏体验模式、默认生成长度和 AI 自由度。作者工具和“我的角色”要保持边界清晰：作者创建故事世界和世界内角色，读者创建自己的入戏身份。
...
- `/admin/stories`：故事、世界、角色、锚点配置查看，MVP 可支持故事基础信息编辑。
- `/admin/sessions`：最近会话与回合审计。
- `/admin/moderation`：审核事件占位。

MVP 管理后台以治理、调试和审核为主，可保留模型配置、Provider 验证和故事基础信息编辑；复杂作者编辑能力应进入 Creator Studio。
...
- MVP 预留：`credit_accounts`、`credit_ledger_entries`、`generation_usage`
- Phase 2 完整激励：`creator_rewards`、`creator_payouts`、`anti_fraud_events`
...
9. 实现故事创建表单和 `POST /api/admin/stories`，支持最小故事草稿。
10. 实现短期上下文、章节摘要和回溯节点。
11. 实现极简管理后台：状态、模型配置、故事配置、会话审计、审核占位。
12. 实现极简作者工具。
13. 加入基础安全审核、积分消耗预留和可观测日志。
