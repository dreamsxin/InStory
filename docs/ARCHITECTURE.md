...
MVP 过渡实现可先用 `TurnInputType = "read_continue"` 表达阅读推进意图，但运行时必须把它当作 `read_segment` 处理：不把按钮文案当成角色输入，不要求模型回应这句话，而是基于故事大纲、最近正文、世界状态和剧情锚点生成下一段小说小节。
...
实现状态（2026-09-12）：第 1 级已落地——`story_segments` 存作者写的小节，`session_turns.segment_id` 记这一回合发的是哪一段，命中条件是「`scripted` 模式 + `read_continue` + 本局还没发过的第一段」，命中时不调模型、不写 `generation_usage`、不扣配额。第 2 级仍然没有，而且是有意先不做的：预设正文落地之后它剩下的命中场景（跨读者的开场几段、回溯重走同一段）都带着产品决定——回溯之后该不该重播一模一样的文字、命中算不算配额——理由记在 `PROGRESS.md` 的下一步第 5 条。第 3 级是默认路径。
