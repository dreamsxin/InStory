...
export const migrations: Migration[] = [
  ...
  {
    id: 15,
    name: "add_story_segments",
    up: `
      -- Passages the author wrote out in full, in the order they are handed to the
      -- reader. Until now an author could only write anchors - constraints on what the
      -- model generates - so "预设正文优先" in ARCHITECTURE.md had no data behind it.
      --
      -- payload holds the passage itself (title, narration, anchorId), and seq holds
      -- the author's order, for the same reason anchors got a seq column: the id is
      -- random so that a saved row keeps its identity across an edit, which means the
      ...
      CREATE INDEX IF NOT EXISTS idx_story_segments_story
      ON story_segments(story_id, seq);
    `
  },
  {
    id: 16,
    name: "add_turn_segment",
    up: `
      -- Which preset passage a turn served, or null for a generated one. This is what
      -- makes "the next unread passage" answerable: the reading picks the first passage
      -- of the story that no turn in this session has served yet.
      --
      -- Nullable and never backfilled: every turn written before this column was
      -- generated by the model, which is exactly what null means here. It also records,
      -- for accounting, the turns that cost nothing - a preset passage calls no model.
      ALTER TABLE session_turns ADD COLUMN segment_id TEXT;

      CREATE INDEX IF NOT EXISTS idx_session_turns_segment
      ON session_turns(segment_id);
    `
  }
